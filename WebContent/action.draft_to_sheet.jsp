...
If all preconditions are fulfilled, then exercise files are created and its contents
...
* The references are eventually turned into real exercises by inserting them as semi-doubles
  ...
  making real of referenced/assigned exercises. (Thus is_native_format would differ too, while
  the plainText content remains equal. Even more important: The exercise filelink will then be
  different as it's a different format than the original exercise fileformat! Hence it's then no
  longer possible to let the exercises point to other sheetdrafts' exercise filesystem exercisefiles.)
...
for changing the content of exercises later on -- The originsheetdraft is stored
