...
/* ======= LOCAL ATTRIBUTES ======= */
String[] downloads = { "EMSy_all", "EMSy__apache_tomcat", "aufgaben_db", "EMSy_data" };
String archive_type = "zip";//7z;
String[] downloadsExpanded = new String[downloads.length];
// expand:
int i = -1;
while (++i < downloads.length) {
	downloadsExpanded[i] =/* "EMSy__" + */downloads[i] + "." + (downloads[i].contains("program") || downloads[i].equals("aufgaben_db") ? "war" : archive_type);
}
...
//	 String mysql2sqlite_conversion_filelink = Global.root + File.separator + "mysql2sqlite.sql";
	...
	// 3 & 4  i.e. the Tomcat container and the Program itself are not generated on the fly.
