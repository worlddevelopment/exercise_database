...
sql = "SELECT DISTINCT exercise.filelink, exercise.* FROM exercise";
...
String sort_by_column = "whencreated";//whenchanged";
...
if (res1 == null) {
    ...
    while (res1.next()) {
        ...
        if (whencreated > 0) {
            ...
            /*
            Date date = new Date(whencreated_in_milliseconds);
            */
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm"//:ss, SSS"
                    , Locale.GERMAN
            );
            GregorianCalendar calendar = new GregorianCalendar(/*TimeZone.getTimeZone("GERMAN")*/);
            calendar.setTimeInMillis(whencreated * 1000);/*<-- seems to be stored in seconds*/
            ...
            while (++ands_index < ands.length) {
                ...
                */
                ...
                while (++versions_i < versions.length) {
                    ...
                    if ( textOnly.contains(versions[versions_i]) ) {
...
if (!globals.tablesorter_event_already_added) {
    initSortBy(".tablesorter", 5, 1);
}
