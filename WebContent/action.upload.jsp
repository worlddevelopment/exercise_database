...
<%
	...
	int aufg_anz = 0;
	String[] aufg_sheetdraft_id = null;
	String[] aufg_filelink = null;
	String[] aufg_originsheetdraft_filelink = null;
	String[] aufg_splitby = null;
	String[] aufg_content = null;
	...
	try {
	    ...
	    /*..........................................................................
	       ...
	       GUARANTEED THAT THE <form>-sent FILE IS ALWAYS LAST IN THE ORDER. 
	     */
        ...
        String[] sheetdraft_plainText = sheetdraft.getPlainText();
        content = Global.getInhalt_as_String(sheetdraft_plainText);
        //out.print(content);
        ...
        //fuer aufgaben
        aufg_anz = al.size();
        Global.addMessage("" + aufg_anz + " Aufgaben gefunden.</br>",
        		aufg_anz == 0 ? "nosuccess" : "success");
        aufg_filelink = (String[])(sheetdraft.getAllExercises().keySet().toArray());
        
        //erzeuge Bilder für Aufgabe
        Global.addMessage("Creating images to each exercise ...", " success "/*, request, pageContext*/);
        for (int i = 0; i < aufg_anz; i++) {
            ...
            Global.createImageFromText(
                    plainText,
                    1024, /*width*/
                    Global.root + "/uploads" + dir + "/",
                    "" + aufg_filelink + "__" + (i + 1) + "__" + ext,
                    "jpg"
                    );
            aufg_content[i] = Global.getInhalt_as_String(plainText);
            doc.add(new Field(aufg_filelink[i], aufg_content[i], Field.Store.YES, Field.Index.ANALYZED));
        }
        ...
        try {
            //hopefully in this Thread appens the creation of the proper html-file
            //which we need so much for extraction of single exercises and re-fusion again.
    ...
    */
	...
	if (!Global.sqlm.exist("sheetdraft", "filelink", column_value)) {
		...
		String message = "<b>Hochgeladene Dateien: </b><br /><br />"
		...
		for(int i = 0; i < aufg_anz; i++) {	
			...
			query = "INSERT INTO exercise "
		        + "(sheetdraft_id, filelink, `originsheetdraft_filelink`, `splitby`, `content`, is_native_format)"
	               + "VALUES ("
		        + "'"   + aufg_sheetdraft_id[i] + "'"
				+ ",'" + aufg_filelink[i] + "'"
				+ ", NULL"/*"','" + aufg_originsheetdraft_filelink[i]*/
				+ ",'" + aufg_splitby[i] + "'"
				+ ",'" + Global.sqlm.mysql_real_escape_string(aufg_content[i])
			    + ", 1" /*is_native_format <-- all directly uploaded sheets are initially in native format*/
			    /*whenchanged - never changed so far*/
			    /*whencreated - automatically given the current db date*/
	            + ");";
			Global.addMessage("query: " + query, "info");
			//execute
			Global.sqlm.executeUpdate(query);
		    message += "<a href='" + aufg_filelink[i] + "' class = 'screenshot' rel='"
			+ Global.getImageLinkFromFile(aufg_filelink[i]) + "'>" + aufg_filelink[i] + "</a></br>";
