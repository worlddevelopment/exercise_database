 ...
 * May be called from the command line as
 * 'PDFToImageUsingPDFView <pdffile> <targetDirForGeneratedImages>'.
...
public class PDFToImageUsingPDFView implements IConverter {
	...
	public static void main(String[] args) {
		if (args.length != 2) {
			System.err.println("Usage: " + PDFToImageUsingPDFView.class
					+ " <pdffile> <targetDirectoryForGeneratedImages>");
			return;
		}
		...
		if (!sourceFile.exists()) {
			...
			System.err.println("Usage: Pdf2Image <pdffile>"
					+ " <targetDirectoryForGeneratedImages>");
		}
	...
	public static void convert(File sourceFile, File targetDir) {
		...
		try {
			...
			for(int i = 0; i < num; i++)
			{
				...
				// Get width, height for the doc at the default zoom
				...
				ImageIO.write(img, "png", new File(targetDir
	...
	public static void convert(File sourceFile, File targetDir
		...
		try {
			...
			for (int i = 0; i < pdf.getNumPages(); i++) {
				...
				Image image = page.getImage(width, height, rect
						, null, true, true );
				Graphics2D bufImageGraphics = bufferedImage.createGraphics();
				bufImageGraphics.drawImage(image, 0, 0, null);
				ImageIO.write(bufferedImage, "png", new File(targetDir
