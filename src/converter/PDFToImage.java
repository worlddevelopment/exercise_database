...
/**
 * TODO see how xdocreport french colleagues program their converters.
 * Also use automatic registering of source-target pairs according
 * according to converters found in the converter package. File.list()?
 */
public class PDFToImage extends Converter implements IConverter{

	/**
	 * TODO this is too static => A more dynamic solution?
	 */
	private static final String /*DocumentKind*/ to = "pdf";
	...
	public static void convert(File sourceFile
			, File targetDirOrFile, Options options) {

		String target_type = "jpg";
		if (options != null && options.getTo() != null
				&& options.getTo() != "") {
			target_type = options.getTo();
		}

		if (targetDirOrFile == null) {
			targetDirOrFile = new File(Global.replaceEnding(
						sourceFile.getAbsolutePath(), target_type));
		}
		...
		if (!targetDirOrFile.isDirectory()) {
			...
			// TODO Investigate: This filepath might still specify
			// a path without ending.
			target_type
				= Global.extractEnding(targetDirOrFile.getAbsolutePath());
		}
		...
		try {
			...
			/*
			Using GHOST4J (having all the native libraries (.dll, .so, ..)
			on the classpath.
			As there occurred problems it might be required to compile
			for other operating systems. For reference see:
			...
			*/
			//Load the file:
			org.ghost4j.document.PDFDocument document
				= new org.ghost4j.document.PDFDocument();
			document.load(sourceFile);

			// Setup the Ghost renderer:
			org.ghost4j.renderer.SimpleRenderer renderer
				= new org.ghost4j.renderer.SimpleRenderer();
			...
			int images_i = -1;
			String image_filelink_base = Global
				.replaceEnding(targetFile.getAbsolutePath(), "");

			while (++images_i < images.size()) {
				if (images_i > 0) {
					ImageIO.write((RenderedImage)images.get(images_i)
							//.getScaledInstance(width
							//, images.get(images_i)
							//.getHeight(null)*getWidth/1024
							//, Image.SCALE_FAST)
							, target_type
							, new File(image_filelink_base
								+ (images_i + 1)
		...
		if (targetFile.exists()) {
			System.out.println("[success-ghost4j] Target file (PDF first"
					+ " page as image) " + targetFile.getAbsolutePath());
			return ;
		}
		...
		*/
		...
		try {
//			document = PDDocument.loadNonSeq(sourceFile
//					, new org.apache.pdfbox.io.RandomAccessFile(
//						new File(sourceFile.getAbsolutePath()
//							+ "__tmp__pdf-creation"), "rw"));
		...
		if (document != null) {
			try {
				...
				boolean first_page_only_condition = options != null
					...
					&& options.getProperty("convertFirstPageOnly")
					instanceof Short
					&& (short)options.getProperty("convertFirstPageOnly")
					== 1;
				...
				while (++page_i < length) {
					// A) following
					// http://stackoverflow.com/questions/20862335/pdfbox-converttoimage-not-rendering-some-pdfs-correctly
					//ImageIO.write((RenderedImage) pages.get(page_i)
					//	.convertToImage(), "png"
					//	, new File("pdf__page_" + (page_i + 1) + ".png"));

					// B) following PDFRenderer source code (strange it
					// seems non-existent in pdfbox<version>.jar!)
					org.apache.pdfbox.rendering.PDFRenderer renderer
						= new org.apache.pdfbox
						.rendering.PDFRenderer(document);

					BufferedImage bufImg = renderer
						.renderImage(page_i, 1.0f, ImageType.RGB);
					// <- JPG (as AlphaRGB is rather PNG)

					// Only for the first page the initial target
					// filelink as given per parameter will be used:
					if (targetFile == null || page_i > 0) {
						// Derive filename from source file:
						// e.g. Aufgabe_1__1.jpg for page 1 and so forth.
						// TODO Will this ever be reached?
						String file =  sourceFile.getName() + "__"
							+ (page_i + 1) + "." + target_type;
						//TODO Check if ImageType above in renderImage changes.
						targetFile = new File(targetDir.getAbsolutePath()
				...
				if (first_page_only_condition) {
					System.out.println("Total pages converted: "
							+ page_i + " (first page only).");
				}
				else {
					System.out.println("Total pages converted: "
							+ page_i);
					// Separation by : makes automatic parsing easier.
				}
