...
/**
Functionality to convert from HTML file to ODT.
The HTML code will be run through Tidy to ensure that it
is valid and well formed XHTML.
TODO Further conver from Python to Java.
*/
public class HTML2ODT {
	...
	class HTMLFile {
		/**
		An OptionParser-result containing options for processing.
		*/
		String[] options;
		/**
		HTML string
		...
		*/
		...
		/**
		Read the HTML file from `options`.input, run it through Tidy,
		...
		*/
		void cleanup() {
			...
			String[] tidy_options = {
				...
				, indent=1
				, tidy_mark=0
				//, input_encoding=str(this.charset)
			...
			*/
			if (this.html == null) {
				throw Exception(
						"Tidy could not clean up the HTML, aborting.");
			}
			...
			*/
			try {
				...
				throw Exception("The XHTML is still not valid after"
						+ " Tidy's work. It can not be converted.");
			}
