 ...
 * TODO Pass the recursive populateMethod the zipStream due to many files?
 * Aim: Saving the extra loop and the heavy memory usage to store all
 * files in a list which also involves copying if the list is no longer
 * big enough and has to be enlarged.
 * This would allow to get rid of static members too.
 */
public class Zip {
...
//	}
	...
	public static boolean zip(File file, String zipTargetFilelink) {
		...
		if (new File(zipTargetFilelink).exists()) {
			return true;// TODO No guarantee the zip file is not empty.
		}
		return false;
	...
	private void zipDirectory(File dir, String zipTargetFilelink) {
		try {
			...
			for(String filePath : filesListInDir){
				...
				// Ffor ZipEntry we need to keep only relative file
				// path, so we used substring on absolute path
				ZipEntry ze = new ZipEntry(filePath.substring(
	...
	/**
	 * populates all the files in a directory to a List
	 * @param dir
	 * @throws IOException
	 */
	...
	/**
	 * compresses the single file to zip format
	 * @param file
	 * @param zipFileName
	 */
